...
# Translated from BASIC to Perl by Alex Kapranoff
# Translated from Perl to Raku by Tom Walsh
...
sub input_dimensions {
    ...
    while $w <= 1 || $h <= 1 {
        say 'WHAT ARE YOUR WIDTH AND LENGTH? ';
		if $*IN.get ~~ /(\d+)\s+(\d+)/ {
			$w = $/[0].Int;
			$h = $/[1].Int;
		}
